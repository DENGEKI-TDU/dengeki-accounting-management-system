import { Tr,Td, Select, Button, HStack, Input, Text, useToast, Tooltip,  AlertDialog,
	AlertDialogBody,
	AlertDialogFooter,
	AlertDialogHeader,
	AlertDialogContent,
			...
			<>
				...
				<Td>
					<HStack>
						...
						<Tooltip label="編集を保存">
							<CheckIcon onClick={() => adopt()} marginRight={"5px"} marginLeft={"5px"} />
						</Tooltip>
						<Tooltip label="変更を破棄">
							<CloseIcon onClick={() => close()} marginRight={"5px"} marginLeft={"5px"} />
						</Tooltip>
						<Tooltip label="支払いを却下">
							<DeleteIcon onClick={onOpen} marginRight={"5px"} marginLeft={"5px"} />
						</Tooltip>
					</HStack>
				</Td>
				<AlertDialog
				...
				>
					...
					<AlertDialogCloseButton />
					<AlertDialogBody>
					{Number(new Date(update.date).getMonth())+1+"月"+String(Number(new Date(update.date).getDate()))+"日"}の{fixture}(¥{update.income != 0 ? update.income:update.outcome})の{update.income != 0 ? "収入":"支出"}を却下しますか？
					</AlertDialogBody>
					<AlertDialogFooter>
